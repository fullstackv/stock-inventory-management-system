...
const Header = ({ setMobileOpen, user }) => {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 shrink-0 items-center gap-3 border-b border-black/5 bg-white/80 px-4 backdrop-blur-xl dark:border-white/10 dark:bg-ink-900/80 sm:px-6">
      ...
      {/* Everything below lives in ONE right-aligned group, so it's always
          flush against the end of the header - regardless of whether the
          search box or notification bell are present (storekeeper) or not
          (owner). Previously, ml-auto was applied per-item, which meant
          the group's actual right edge depended on which items happened
          to render for a given role instead of being pinned reliably. */}
      <div className="ml-auto flex items-center gap-2 sm:gap-3">
          ...
          <form onSubmit={handleSearchSubmit} className="hidden sm:block">
            <div className="relative">
              ...
              <input
                ...
                className="input-field !py-2 w-56 pl-9 text-sm lg:w-72"
              />
            </div>
          </form>
        )}
        ...
        <button
          onClick={toggleTheme}
          className="rounded-lg p-2 text-ink-700 transition-colors hover:bg-slate-100 dark:text-white/70 dark:hover:bg-white/10"
